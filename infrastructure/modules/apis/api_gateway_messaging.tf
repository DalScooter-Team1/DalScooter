...
# Customer Messages endpoint - uses customer authorizer (customers can get their own messages)
resource "aws_api_gateway_resource" "customer_messages" {
  rest_api_id = aws_api_gateway_rest_api.dalscooter_apis.id
  parent_id   = aws_api_gateway_rest_api.dalscooter_apis.root_resource_id
  path_part   = "customer-messages"
}

resource "aws_api_gateway_method" "customer_messages_get" {
  rest_api_id   = aws_api_gateway_rest_api.dalscooter_apis.id
  resource_id   = aws_api_gateway_resource.customer_messages.id
  http_method   = "GET"
  authorization = "NONE"
  # Temporarily disable authorizer for debugging CORS issues
  # authorizer_id = aws_api_gateway_authorizer.customer_authorizer.id
}
...
resource "aws_api_gateway_integration" "customer_messages_integration" {
  rest_api_id             = aws_api_gateway_rest_api.dalscooter_apis.id
  resource_id             = aws_api_gateway_resource.customer_messages.id
  http_method             = aws_api_gateway_method.customer_messages_get.http_method
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  uri                     = var.get_customer_messages_lambda_invoke_arn
}
...
# Customer Messages - OPTIONS method for CORS
resource "aws_api_gateway_method" "customer_messages_options" {
  rest_api_id   = aws_api_gateway_rest_api.dalscooter_apis.id
  resource_id   = aws_api_gateway_resource.customer_messages.id
  http_method   = "OPTIONS"
  authorization = "NONE"
...
resource "aws_api_gateway_integration" "customer_messages_options_integration" {
  rest_api_id = aws_api_gateway_rest_api.dalscooter_apis.id
  resource_id = aws_api_gateway_resource.customer_messages.id
  http_method = aws_api_gateway_method.customer_messages_options.http_method
...
resource "aws_api_gateway_method_response" "customer_messages_options_response" {
  rest_api_id = aws_api_gateway_rest_api.dalscooter_apis.id
  resource_id = aws_api_gateway_resource.customer_messages.id
  http_method = aws_api_gateway_method.customer_messages_options.http_method
  status_code = "200"
...
resource "aws_api_gateway_integration_response" "customer_messages_options_integration_response" {
  rest_api_id = aws_api_gateway_rest_api.dalscooter_apis.id
  resource_id = aws_api_gateway_resource.customer_messages.id
  http_method = aws_api_gateway_method.customer_messages_options.http_method
  status_code = aws_api_gateway_method_response.customer_messages_options_response.status_code
...
# Customer Messages - GET method response for CORS
resource "aws_api_gateway_method_response" "customer_messages_get_response" {
  rest_api_id = aws_api_gateway_rest_api.dalscooter_apis.id
  resource_id = aws_api_gateway_resource.customer_messages.id
  http_method = aws_api_gateway_method.customer_messages_get.http_method
  status_code = "200"
...
resource "aws_api_gateway_integration_response" "customer_messages_get_integration_response" {
  rest_api_id = aws_api_gateway_rest_api.dalscooter_apis.id
  resource_id = aws_api_gateway_resource.customer_messages.id
  http_method = aws_api_gateway_method.customer_messages_get.http_method
  status_code = aws_api_gateway_method_response.customer_messages_get_response.status_code

  response_parameters = {
    "method.response.header.Access-Control-Allow-Origin" = "'*'"
  }

  depends_on = [aws_api_gateway_integration.customer_messages_integration]
}
