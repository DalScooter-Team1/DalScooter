# ================================
# BOOKING DATA PIPELINE MODULE
# ================================
# This module contains the Lambda function that syncs DynamoDB booking data to MySQL
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = var.booking_table_stream_arn
      },
      {
        Effect = "Allow"
        Action = [
...
resource "aws_lambda_function" "booking_data_pipeline" {
  ...
  # MySQL layer for PyMySQL dependencies
  ...
  environment {
    variables = {
      MYSQL_HOST     = var.rds_endpoint
      MYSQL_PORT     = tostring(var.rds_port)
      MYSQL_DATABASE = var.rds_database_name
  ...
  depends_on = [
    ...
    aws_lambda_layer_version.mysql_layer
  ]
...
resource "aws_security_group" "lambda_rds_sg" {
  ...
  tags = {
    Name    = "DALScooter Lambda RDS Security Group"
    Project = "DALScooter"
  }
}
...
resource "aws_lambda_event_source_mapping" "booking_stream_trigger" {
  event_source_arn  = var.booking_table_stream_arn
  function_name     = aws_lambda_function.booking_data_pipeline.arn
  starting_position = "LATEST"
  batch_size        = 10
  enabled           = true

  depends_on = [aws_lambda_function.booking_data_pipeline]
}
